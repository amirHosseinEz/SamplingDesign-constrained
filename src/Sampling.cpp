...
//            string nucij = idx_to_nucs[{idx, pos.size()}];
            ...
            string nucij;
            if (this->constrained) {
                nucij = this->allowed_options[pos][idx];
            } else {
                nucij = idx_to_nucs[{idx, pos.size()}];
            }
...
//            string nucij = idx_to_pairs[{idx, pos.size()}];
            ...
            if (this->constrained) {
                ...
                nucij = idx_to_pairs[{idx, pos.size()}];
            }
            for (int x = 0; x < pos.size(); x++) {
                seq[pos[x]] = nucij[x];
            }
...
//            int idx = nucs_to_idx[nucij];
            ...
            int idx;
            if (this->constrained) {
                idx = this->options_to_idx[pos][nucij];
            } else {
                idx = nucs_to_idx[nucij];
            }
            sample_prob *= probs[idx];
        }
    ...
    // Approximate gradient
    {
        for (const Sample& sample: samples) {
            for (const vector<int>& pos: unpaired_pos) {
                ...
                // NEW: Get the index based on the mode
                int idx;
                if (this->constrained) {
                    idx = this->options_to_idx[pos][nucij];
                } else {
                ...
                if (importance) {
                    gradient[pos][idx] += sample.obj * (1 / dist[pos][idx]) * (sample.sample_prob / sample.old_sample_prob);
                } else {
                    gradient[pos][idx] += sample.obj * (1 / dist[pos][idx]);
                }
            }
