...
#define MIN_CUBE_PRUNING_SIZE 20
  ...
  #define LF_VALUE_MIN std::numeric_limits<int>::lowest()
#else
  typedef double value_type;
  #define LF_VALUE_MIN std::numeric_limits<double>::lowest()
#endif
...
struct LF_State {
    ...
    LF_State(): manner(MANNER_NONE), score(LF_VALUE_MIN) {};
    LF_State(value_type s, Manner m): score(s), manner(m) {};
...
private:
    ...
    std::pair<std::string, std::string> get_parentheses_outside_real_backtrace(int i, int j, LF_State& state_beta, std::map<std::tuple<BestTypes, int, int>, std::pair<std::string, std::string> >& global_visited_outside, std::map<std::tuple<BestTypes, int, int>, std::string>& global_visited_inside, std::set<std::pair<int,int> >& window_visited);
    std::string get_parentheses_inside_real_backtrace(int i, int j, LF_State& LF_State, std::map<std::tuple<BestTypes, int, int>, std::string>& global_visited_inside, std::set<std::pair<int,int> >& window_visited);

    unsigned seq_length;

    std::vector<std::unordered_map<int, LF_State>> bestH, bestP, bestM2, bestMulti, bestM;

    //Zuker subopt
    std::vector<std::unordered_map<int, LF_State>> bestH_beta, bestP_beta, bestM2_beta, bestMulti_beta, bestM_beta;
    ...
    std::vector<std::pair<int, LF_State>> keys;

    // hzhang: sort keys in each beam to avoid randomness
    void sort_keys(std::unordered_map<int, LF_State> &map, std::vector<std::pair<int,LF_State>> &sorted_keys);

    void sortM(value_type threshold,
               std::unordered_map<int, LF_State> &beamstep,
               std::vector<std::pair<value_type, int>>& sorted_stepM);
    std::vector<LF_State> bestC;
    std::vector<int> nucs;

    //Zuker subopt
    std::vector<LF_State> bestC_beta;
    ...
    void update_if_better(LF_State &LF_State, value_type newscore, Manner manner) {
      if (LF_State.score < newscore)
            LF_State.set(newscore, manner);
    };

    void update_if_better(LF_State &LF_State, value_type newscore, Manner manner, int split) {
        if (LF_State.score < newscore || LF_State.manner == MANNER_NONE)
            LF_State.set(newscore, manner, split);
    };

    void update_if_better(LF_State &LF_State, value_type newscore, Manner manner, char l1, int l2) {
        if (LF_State.score < newscore || LF_State.manner == MANNER_NONE)
            LF_State.set(newscore, manner, l1, l2);
    };

    value_type beam_prune(std::unordered_map<int, LF_State>& beamstep);
