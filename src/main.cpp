...
string GradientDescent::get_max_probability_solution() {
    ...
    for (const vector<int>& pos: base_pairs_pos) {
        ...
        if (this->constrained) {
            ...
            winning_pair = idx_to_pairs[{nucij, pos.size()}]; 
        }
        for (int x = 0; x < pos.size(); x++) {
...
int main(int argc, char** argv){
    std::cerr << "--- C++ EXECUTABLE HAS STARTED ---" << std::endl; 
    ...
    if (mode == "eval") {
        ...
        cerr << "DEBUG: C++ program entered ncrna_design mode." << endl; 

        for (string rna_struct; getline(cin, rna_struct);){
            cerr << "DEBUG: C++ loop read a line: " << rna_struct << endl; 
    ...
    cerr << "DEBUG: C++ program finished input loop." << endl; 
    } else {
