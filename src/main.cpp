...
GradientDescent::GradientDescent(string rna_struct,
    ...
    : rna_struct(rna_struct),
      iupac_constraint(iupac_constraint),
...
//            seq[pos[x]] = idx_to_nucs[{nucij, pos.size()}][x];
        // edited for constrained mod
    //    if (this->constrained) {
  //          winning_choice = this->allowed_options[pos][nucij];
      //  } else {
        //    winning_choice = idx_to_nucs[{nucij, pos.size()}];
       // }
       // for (int x = 0; x < pos.size(); x++) {
         //   seq[pos[x]] = winning_choice[x];
        //}
    //}
...
string GradientDescent::get_max_probability_solution() {
    ...
    for (const vector<int>& pos: unpaired_pos) {
        ...
        string winning_choice; // <-- FIX: Declare the variable
        if (this->constrained) {
            winning_choice = this->allowed_options[pos][nucij];
        } else {
            winning_choice = idx_to_nucs[{nucij, pos.size()}];
    ...
    for (const vector<int>& pos: base_pairs_pos) {
        ...
        string winning_pair; // <-- FIX: Declare the variable
        if (this->constrained) {
            winning_pair = this->allowed_options[pos][nucij];
        } else {
            winning_pair = idx_to_pairs[{nucij, pos.size()}]; // <-- FIX: Use idx_to_pairs
        }
        for (int x = 0; x < pos.size(); x++) {
            seq[pos[x]] = winning_pair[x];
        }
    }
...
void GradientDescent::initialize_dist_no_mismatch_constrained() {
    // Rewritten to handle IUPAC constraints for the simple v1 distribution.

    // === STAGE 1: Parse Structure (Simple Version) ===
    stack<int> stk;
    for (int j = 0; j < rna_struct.size(); j++) {
        if (rna_struct[j] == '(') {
            stk.push(j);
    ...
    // === NEW STAGE 2: Generate Allowed Options Based on IUPAC Constraints ===
    map<char, string> iupac_map = {
            {'A', "A"}, {'C', "C"}, {'G', "G"}, {'U', "U"}, {'R', "AG"}, {'Y', "CU"},
            {'S', "GC"}, {'W', "AU"}, {'K', "GU"}, {'M', "AC"}, {'B', "CGU"}, {'D', "AGU"},
            {'H', "ACU"}, {'V', "ACG"}, {'N', "ACGU"}, {'.', "ACGU"}
    };
    vector<string> valid_pairs = {"CG", "GC", "GU", "UG", "AU", "UA"};

    // Process all unpaired groups (which are all size 1)
    for (const auto& pos : unpaired_pos) {
        char iupac_code = this->iupac_constraint[pos[0]];
        string allowed_chars = iupac_map[iupac_code];
        for (char c : allowed_chars) {
            this->allowed_options[pos].push_back(string(1, c));
        }
    }

    // Process all base pair groups
    for (const auto& pos : base_pairs_pos) {
        string allowed1 = iupac_map[this->iupac_constraint[pos[0]]];
        string allowed2 = iupac_map[this->iupac_constraint[pos[1]]];
        for (const string& pair : valid_pairs) {
            if (allowed1.find(pair[0]) != string::npos && allowed2.find(pair[1]) != string::npos) {
                this->allowed_options[pos].push_back(pair);
            }
        }
    }

    // === NEW STAGE 3: Initialize Distributions with Dynamic Sizes ===
    // This part is identical to the final stage of the main initialize_dist function.
    for (const auto& [pos, options] : this->allowed_options) {
        int num_options = options.size();
        if (num_options == 0) {
            string pos_str = ""; for(int p : pos) pos_str += to_string(p) + " ";
            throw std::runtime_error("No valid options for position group " + pos_str + "due to IUPAC constraints.");
        }

        for (int i = 0; i < num_options; ++i) {
            this->options_to_idx[pos][options[i]] = i;
        }

        if (init == "uniform") {
            if (softmax) {
                logits[pos] = vector<double>(num_options, 0.0);
            } else {
                dist[pos] = vector<double>(num_options, 1.0 / num_options);
            }
        } else if (init == "targeted") {
            vector<double> initial_probs(num_options, 0.0);

            bool is_unpaired_group = find(unpaired_pos.begin(), unpaired_pos.end(), pos) != unpaired_pos.end();

            if (is_unpaired_group) { // Targeted for single unpaired nucs
                auto it = find(options.begin(), options.end(), "A");
                if (it != options.end()) initial_probs[distance(options.begin(), it)] = 1.0;
                else fill(initial_probs.begin(), initial_probs.end(), 1.0 / num_options);
            } else { // Targeted for base pairs with tiered priority
                auto it_cg = find(options.begin(), options.end(), "CG");
                auto it_gc = find(options.begin(), options.end(), "GC");
                vector<decltype(it_cg)> tier1_found;
                if (it_cg != options.end()) tier1_found.push_back(it_cg);
                if (it_gc != options.end()) tier1_found.push_back(it_gc);
                if (tier1_found.size() > 0) {
                    for (const auto& it : tier1_found) initial_probs[distance(options.begin(), it)] = 1.0 / tier1_found.size();
                } else {
                    auto it_au = find(options.begin(), options.end(), "AU");
                    auto it_ua = find(options.begin(), options.end(), "UA");
                    vector<decltype(it_au)> tier2_found;
                    if (it_au != options.end()) tier2_found.push_back(it_au);
                    if (it_ua != options.end()) tier2_found.push_back(it_ua);
                    if (tier2_found.size() > 0) {
                        for (const auto& it : tier2_found) initial_probs[distance(options.begin(), it)] = 1.0 / tier2_found.size();
                    } else {
                        fill(initial_probs.begin(), initial_probs.end(), 1.0 / num_options);
                    }
                }
            }

            for (int i = 0; i < num_options; ++i) {
                initial_probs[i] = initial_probs[i] * eps + (1.0 / num_options) * (1.0 - eps);
            }

            if (softmax) {
                for (double p : initial_probs) logits[pos].push_back(log(p));
            } else {
                dist[pos] = initial_probs;
            }
        }
    }
}
...
void GradientDescent::initialize_dist_constrained() {
    // This function is now rewritten to handle IUPAC constraints.

    // === STAGE 1: Parse Structure (This part is the same as before) ===
    // This part still correctly identifies the position groups based on the structure.
    if (!mismatch) {
        initialize_dist_no_mismatch();
        return;
    }
    ...
    // === NEW STAGE 2: Generate Allowed Options Based on IUPAC Constraints ===
    // Helper data structures for IUPAC codes and valid pairs.
    map<char, string> iupac_map = {
            {'A', "A"}, {'C', "C"}, {'G', "G"}, {'U', "U"}, {'R', "AG"}, {'Y', "CU"},
            {'S', "GC"}, {'W', "AU"}, {'K', "GU"}, {'M', "AC"}, {'B', "CGU"}, {'D', "AGU"},
            {'H', "ACU"}, {'V', "ACG"}, {'N', "ACGU"}, {'.', "ACGU"} // Treat '.' as 'N'
    };
    vector<string> valid_pairs = {"CG", "GC", "GU", "UG", "AU", "UA"};

    // Process all unpaired/mismatch/trimismatch groups
    for (const auto& pos : unpaired_pos) {
        vector<string> current_options;
        // Recursive function to generate all permutations for the group
        function<void(int, string)> generate_perms =
                [&](int k, string current_string) {
                    if (k == pos.size()) {
                        current_options.push_back(current_string);
                        return;
                    }
                    char iupac_code = this->iupac_constraint[pos[k]];
                    string allowed_chars = iupac_map[iupac_code];
                    for (char c : allowed_chars) {
                        generate_perms(k + 1, current_string + c);
                    }
                };
        generate_perms(0, "");
        this->allowed_options[pos] = current_options;
    }
    ...
    // === NEW STAGE 3: Initialize Distributions with Dynamic Sizes ===
    // This loop now iterates over our newly created allowed_options map.
    for (const auto& [pos, options] : this->allowed_options) {
        int num_options = options.size();
        if (num_options == 0) {
            // This is an error case: the constraints are impossible to satisfy.
            string pos_str = "";
            for(int p : pos) pos_str += to_string(p) + " ";
            throw std::runtime_error("No valid options for position group " + pos_str + "due to IUPAC constraints.");
        }

        // Create the reverse lookup map (string to index) for this position
        for (int i = 0; i < num_options; ++i) {
            this->options_to_idx[pos][options[i]] = i;
        }

        // Initialize distributions based on the dynamic `num_options`
        if (init == "uniform") {
            if (softmax) {
                logits[pos] = vector<double>(num_options, 0.0);
            } else {
            ...
            // Check if the position group is in unpaired_pos or base_pairs_pos
            bool is_unpaired_group = find(unpaired_pos.begin(), unpaired_pos.end(), pos) != unpaired_pos.end();

            if (pos.size() == 1) { // Targeted for single unpaired nucs
                auto it = find(options.begin(), options.end(), "A");
                if (it != options.end()) initial_probs[distance(options.begin(), it)] = 1.0;
                else fill(initial_probs.begin(), initial_probs.end(), 1.0 / num_options); // Fallback to uniform
                // in main.cpp, inside initialize_dist()
            } else if (!is_unpaired_group) { // Targeted for base pairs with TIERED priority
                // Tier 1: Prioritize CG and GC
                auto it_cg = find(options.begin(), options.end(), "CG");
                auto it_gc = find(options.begin(), options.end(), "GC");
                ...
                int tier1_count = tier1_found.size();

                if (tier1_count > 0) {
                    // A high-priority pair was found. Distribute 100% probability among them.
                    for (const auto& it : tier1_found) {
                        initial_probs[distance(options.begin(), it)] = 1.0 / tier1_count;
                    }
                } else {
                    // Tier 2: No CG/GC was found, so now check for AU and UA.
                    auto it_au = find(options.begin(), options.end(), "AU");
                    auto it_ua = find(options.begin(), options.end(), "UA");
                    ...
                    int tier2_count = tier2_found.size();

                    if (tier2_count > 0) {
                        // A second-priority pair was found. Distribute 100% probability among them.
                        for (const auto& it : tier2_found) {
                            initial_probs[distance(options.begin(), it)] = 1.0 / tier2_count;
                        }
                    } else {
                        // Fallback: None of the four preferred pairs are allowed. Use uniform.
                        fill(initial_probs.begin(), initial_probs.end(), 1.0 / num_options);
                    }
                }
            } else { // Mismatches/Trimismatches are always uniform in targeted mode
                fill(initial_probs.begin(), initial_probs.end(), 1.0 / num_options);
            }

            // Apply the epsilon blend to the targeted probabilities
            for (int i = 0; i < num_options; ++i) {
                initial_probs[i] = initial_probs[i] * eps + (1.0 / num_options) * (1.0 - eps);
            }

            // Assign to dist or convert to logits
            if (softmax) {
                for (double p : initial_probs) {
                    logits[pos].push_back(log(p));
                }
            } else {
                dist[pos] = initial_probs;
            }
...
void GradientDescent::print_mode() {
    ...
    cout << "constrined: " << constrained << ", iupac: " << iupac_constraint <<  "\n";
...
void GradientDescent::gradient_descent() {
    ...
    //constrained mod
    if (this->constrained) {
        initialize_dist_constrained();
    } else {
        initialize_dist(); // The original unconstrained version
    }
...
int main(int argc, char** argv){
    std::cerr << "--- C++ EXECUTABLE HAS STARTED ---" << std::endl; // <-- ADD THIS LINE
	...
	constrained = atoi(argv[31]) == 1;
	iupac_constraint = argv[32];
        init = argv[3];
        eps = atof(argv[4]);
    ...
    std::cerr << "before mode check!." << std::endl;
    if (mode == "eval") {
        string rna_struct;
        for (string rna_seq; getline(cin, rna_seq);){
            getline(cin, rna_struct);
	    std::cerr << "inside for check. seq = "<< rna_seq << std::endl;
            if (rna_seq.size() > 0 && rna_struct.size() > 0) {
                ...
                try {
                    ...
                    GradientDescent parser(rna_struct,iupac_constraint, constrained, objective, init, eps, softmax, adam, nesterov, beta_1, beta_2, initial_lr, lr_decay, lr_decay_rate, adaptive_lr, k_ma_lr, lr_decay_step, num_steps, early_stop, k_ma, beamsize, !sharpturn, is_lazy, sample_size, best_k, importance, mismatch, trimismatch, seed, verbose, num_threads, boxplot);
        ...
        cerr << "DEBUG: C++ program entered ncrna_design mode." << endl; // <-- ADD THIS

        for (string rna_struct; getline(cin, rna_struct);){
            cerr << "DEBUG: C++ loop read a line: " << rna_struct << endl; // <-- ADD THIS
    ...
    cerr << "DEBUG: C++ program finished input loop." << endl; // <-- ADD THIS
    } else {
