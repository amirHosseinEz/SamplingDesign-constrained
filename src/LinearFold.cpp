...
#ifdef lpv
    bool comparefunc(std::pair<int,LF_State> a, std::pair<int,LF_State> b) {
        return a.first > b.first;
    }

    void LinearFold::sort_keys(std::unordered_map<int, LF_State> &map, std::vector<std::pair<int,LF_State>> &sorted_keys) {
        sorted_keys.clear();
        for(auto &kv : map) {
...
void LinearFold::get_parentheses(char* result, string& seq) {
    memset(result, '.', seq_length);
    result[seq_length] = 0;

    stack<tuple<int, int, LF_State>> stk;
    ...
    while ( !stk.empty() ) {
        tuple<int, int, LF_State> top = stk.top();
        int i = get<0>(top), j = get<1>(top);
        LF_State& state = get<2>(top);
        stk.pop();
...
value_type LinearFold::beam_prune(std::unordered_map<int, LF_State> &beamstep) {
    scores.clear();
    for (auto &item : beamstep) {
        int i = item.first;
        LF_State &cand = item.second;
        int k = i - 1;
        value_type newscore;
        // lisiz: for _V, avoid -inf-int=+inf
        if ((k >= 0) && (bestC[k].score == LF_VALUE_MIN)) newscore = LF_VALUE_MIN;
        else newscore = (k >= 0 ? bestC[k].score : 0) + cand.score;
        scores.push_back(make_pair(newscore, i));
    }
    if (scores.size() <= beam) return LF_VALUE_MIN;
...
void LinearFold::sortM(value_type threshold,
                          std::unordered_map<int, LF_State> &beamstep,
                          std::vector<std::pair<value_type, int>> &sorted_stepM) {
    sorted_stepM.clear();
    if (threshold == LF_VALUE_MIN) {
        ...
        for (auto &item : beamstep) {
            ...
            int k = i - 1;
            value_type newscore;
            // lisiz: constraints may cause all LF_VALUE_MIN, sorting has no use
            if ((use_constraints) && (k >= 0) && (bestC[k].score == LF_VALUE_MIN)) newscore = cand.score;
            else newscore = (k >= 0 ? bestC[k].score : 0) + cand.score;
...
std::string LinearFold::get_parentheses_inside_real_backtrace(int i, int j, LF_State& state, map<tuple<BestTypes, int, int>, string>& global_visited_inside, set<pair<int,int> >& window_visited) {
...
pair<string, string> LinearFold::get_parentheses_outside_real_backtrace(int i, int j, LF_State& state_beta, map<tuple<BestTypes, int, int>, pair<string, string>>& global_visited_outside, map<tuple<BestTypes, int, int>, string>& global_visited_inside, set<pair<int,int>>& window_visited) {
...
void LinearFold::prepare(unsigned len) {
    seq_length = len;
...
// lisiz, constraints
bool LinearFold::allow_paired(int i, int j, vector<int>* cons, char nuci, char nucj) {
    return ((*cons)[i] == -1 || (*cons)[i] == j) && ((*cons)[j] == -1 || (*cons)[j] == i) && _allowed_pairs[nuci][nucj];
}

LinearFold::DecoderResult LinearFold::parse(string& seq, vector<int>* cons) {
    ...
    for(int j = 0; j < seq_length; ++j) {
        ...
        unordered_map<int, LF_State>& beamstepH = bestH[j];
        unordered_map<int, LF_State>& beamstepMulti = bestMulti[j];
        unordered_map<int, LF_State>& beamstepP = bestP[j];
        unordered_map<int, LF_State>& beamstepM2 = bestM2[j];
        unordered_map<int, LF_State>& beamstepM = bestM[j];
        LF_State& beamstepC = bestC[j];
	    ...
	    // lhuang: check j < n-1
            ...
            for(auto& item : beamstepP) {
                ...
                value_type prev_score = LF_VALUE_MIN;
                value_type current_score = LF_VALUE_MIN;
        ...
        // beam of M
        {
            value_type threshold = LF_VALUE_MIN;
    ...
    LF_State& viterbi = bestC[seq_length-1];
...
void LinearFold::outside(vector<int> next_pair[]){
    ...
    for(int j = seq_length-1; j >= 0; --j) {
        ...
        unordered_map<int, LF_State>& beamstepH_beta = bestH_beta[j];
        unordered_map<int, LF_State>& beamstepMulti_beta = bestMulti_beta[j];
        unordered_map<int, LF_State>& beamstepP_beta = bestP_beta[j];
        unordered_map<int, LF_State>& beamstepM2_beta = bestM2_beta[j];
        unordered_map<int, LF_State>& beamstepM_beta = bestM_beta[j];
        LF_State& beamstepC_beta = bestC_beta[j];
        ...
        // beam of M
        {
            for(auto& item : beamstepM) {
                ...
                int i = item.first;
                LF_State& state = beamstepM_beta[i];
        ...
        // beam of M2
        {
            for(auto& item : beamstepM2) {
                int i = item.first;
                LF_State& state = beamstepM2_beta[i];
        ...
        {  
            for(auto& item : beamstepP) {
                int i = item.first;
                LF_State& state = beamstepP_beta[i];
                ...
                if ( k > 0 && !bestM[k].empty()) {
                    ...
                    for (auto &m : bestM[k]) {
                        int newi = m.first;
                        LF_State& m_state = bestM_beta[k][newi];
        ...
        // beam of Multi
        {
            for(auto& item : beamstepMulti) {
                int i = item.first;
                LF_State& state = beamstepMulti_beta[i];
...
LinearFold::LinearFold(int beam_size,
                             bool nosharpturn,
                             bool verbose,
                             bool constraints,
                             bool zuker_subopt,
...
//                         LinearFold parser(beamsize, !sharpturn, is_verbose, is_constraints, false, 5.0, "", false, dangles);

//                         LinearFold::DecoderResult result = parser.parse(seq, &cons);
...
//                 // lhuang: moved inside loop, fixing an obscure but crucial bug in initialization
//                 LinearFold parser(beamsize, !sharpturn, is_verbose, false, zuker_subopt, energy_delta, shape_file_path, fasta, dangles);
