...
while IFS= read -r line; do
    ...
    # uniform intialization with constraints
    echo "${puzzles[1]}" | ./main --init uniform --boxplot --constrained --iupac "${puzzles[2]}" > results/$folder_1/${puzzles[0]}.txt
    python analysis.py --folder "$folder_1" --file ${puzzles[0]}.txt > ./analysis/$folder_1/${puzzles[0]}.txt

    # epsilon-targeted intialization with constraints
    echo "${puzzles[1]}" | ./main --init targeted --eps 0.75 --boxplot --constrained --iupac "${puzzles[2]}" > results/$folder_2/${puzzles[0]}.txt
