...
def process_result_file(rna_id, result_file, args):
    ...
    for idx, line in enumerate(lines):
        ...
        if line.startswith("KL divergence"):
            kl_div_value = float(line.split(': ')[1])
    ...
    if not args.no_graph:
        if objective == "prob":
            ...
            graph_prob(rna_id, lines, obj, avg_pyx, integral_obj, sampled_obj, boxplot, entropy, kl_div, lr_idx, args)
        else:
            # graph(rna_id, objective, lines, obj, integral_obj, sampled_obj, boxplot, entropy, lr_idx, args)
            graph(rna_id, objective, lines, obj, integral_obj, sampled_obj, boxplot, entropy, kl_div, lr_idx, args)
